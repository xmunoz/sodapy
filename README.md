...
Retrieving data is easy! Use SQL-style keyword args to filter data, or lookup an individual row by simply appending the row identifier to the resource endpoint for that dataset.
    ...
    >>> client.get("nimj-3ivp.json", limit=2)
	...
	>>> client.get("nimj-3ivp.json", where="depth > 300", order="magnitude DESC", exclude_system_fields=False)
    ...
    >>> client.get("nimj-3ivp/193.json", exclude_system_fields=False)
